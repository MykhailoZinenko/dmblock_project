...
import { Bounds } from '../../src/engine/math/Bounds.js';
import { Color } from '../../src/engine/utils/Color.js';
import { EventEmitter } from '../../src/engine/utils/EventEmitter.js';
import { Pool } from '../../src/engine/utils/Pool.js';
...
import { AnimatedSprite } from '../../src/engine/nodes/AnimatedSprite.js';
...
// Pooling comparison
const pool = new Pool(() => ({ x: 0, y: 0 }), (o) => { o.x = 0; o.y = 0; });
registerAll([
...
async function createTestEngine(canvasId) {
...
registerAll([
  gpuBenchmark('[GPU] 1k sprites same texture', async () => {
  ...
  gpuBenchmark('[GPU] 10k sprites same texture', async () => {
    const engine = await createTestEngine('perf-canvas');
    const tex = await engine.textures.load('perf_tower', '../../assets/buildings/blue/tower/tower.png');
  ...
  gpuBenchmark('[GPU] 1k sprites alternating 2 textures (batch breaks)', async () => {
    const engine = await createTestEngine('perf-canvas');
  ...
  gpuBenchmark('[GPU] 100 Graphics hexagons (static)', async () => {
    const engine = await createTestEngine('perf-canvas');
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
  ...
  gpuBenchmark('[GPU] 100 Graphics hexagons (moving)', async () => {
    const engine = await createTestEngine('perf-canvas');
    const gfxNodes = [];
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
  ...
  gpuBenchmark('[GPU] Sprites + Graphics interleaved (zIndex mix)', async () => {
    const engine = await createTestEngine('perf-canvas');
    const tex = await engine.textures.load('perf_tower', '../../assets/buildings/blue/tower/tower.png');
    for (let i = 0; i < 200; i++) {
      if (i % 3 === 0) {
...
function makeStressScene(count) {
  return async () => {
    const engine = await createTestEngine('perf-canvas');
    const tex = await engine.textures.load('perf_tower', '../../assets/buildings/blue/tower/tower.png');
    for (let i = 0; i < count; i++) {
    ...
    return {
      ...
      tickCpuOnly: () => {
        ...
        engine.renderer._renderList.length = 0;
        engine.renderer._collectNodes(engine.stage, engine.camera.getViewportBounds());
        engine.renderer._batcher.beginFrame();
        for (const node of engine.renderer._renderList) {
          if (node instanceof Sprite) engine.renderer._batcher.pushSprite(node);
        }
      },
      tick: () => {
        engine.stage.updateTransform();
...
registerAll([
  stressBenchmark('[Stress] 500 sprites × 500 draw calls (1 texture per sprite)', async () => {
    const engine = await createTestEngine('perf-canvas');
    const textures = [];
    // Create many unique 1x1 textures
    for (let i = 0; i < 500; i++) {
      const gpuTex = engine.renderer._device.createTexture({
        size: [1, 1],
        format: 'rgba8unorm',
        usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST,
      });
      engine.renderer._device.queue.writeTexture(
        { texture: gpuTex },
        new Uint8Array([Math.random() * 255, Math.random() * 255, Math.random() * 255, 255]),
        { bytesPerRow: 4 }, [1, 1],
      );
      const sampler = engine.renderer._device.createSampler({ minFilter: 'nearest', magFilter: 'nearest' });
...
registerAll([
  stressBenchmark('[Stress] 10k dynamic sprites (moving every frame)', async () => {
    const engine = await createTestEngine('perf-canvas');
    const tex = await engine.textures.load('perf_tower', '../../assets/buildings/blue/tower/tower.png');
    const sprites = [];
...
registerAll([
  stressBenchmark('[Stress] Overdraw: 1000 fullscreen transparent quads', async () => {
    const engine = await createTestEngine('perf-canvas');
    const gpuTex = engine.renderer._device.createTexture({
...
registerAll([
  stressBenchmark('[Stress] 1000 Graphics hexagons (moving)', async () => {
    const engine = await createTestEngine('perf-canvas');
    const gfxNodes = [];
    for (let i = 0; i < 1000; i++) {
      const g = new Graphics();
