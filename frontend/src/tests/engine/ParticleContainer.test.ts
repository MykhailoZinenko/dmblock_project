...
import type { Particle } from '../../engine/nodes/ParticleContainer.js';
...
import { BLEND_MODES } from '../../engine/render/BlendMode.js';

function makeTex(w: number = 64, h: number = 64): Texture {
  return new Texture(new BaseTexture(null, null, w, h));
}

function makeParticle(x: number = 0, y: number = 0): Particle {
...
describe('ParticleContainer', () => {
  ...
  it('_rebuild handles rotation (sin/cos)', () => {
    const pc = new ParticleContainer();
    const p: Particle = { x: 0, y: 0, scaleX: 1, scaleY: 1, rotation: Math.PI / 2, alpha: 1, tint: 0xFFFFFF, texture: makeTex() };
    pc.addParticle(p);
    pc._rebuild();
