...
export interface GpuBenchSetup {
...
export interface GpuBench {
  name: string;
  setupFn: () => Promise<GpuBenchSetup>;
  type: 'gpu';
  duration: number;
}

export interface StressBenchSetup {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  engine: any;
  tick: () => void;
  tickCpuOnly?: () => void;
...
export interface StressBench {
  name: string;
  setupFn: () => Promise<StressBenchSetup>;
  type: 'stress';
  frames: number;
}

export type Bench = CpuBench | GpuBench | StressBench;
...
export interface StressResult {
  ...
  cpuSubmitFps: string;
  sustainedFps: string;
  sustainedAvgMs: string;
  sustainedP95Ms: string;
  sustainedP99Ms: string;
  sustainedMaxMs: string;
}

export type BenchResult = CpuResult | GpuResult | StressResult;

export function benchmark(name: string, fn: () => void, iterations: number = 100_000): CpuBench {
  return { name, fn, iterations, type: 'cpu' };
}

export function gpuBenchmark(name: string, setupFn: () => Promise<GpuBenchSetup>, options: { duration?: number } = {}): GpuBench {
  return { name, setupFn, type: 'gpu', duration: options.duration || 5 };
}

export function stressBenchmark(name: string, setupFn: () => Promise<StressBenchSetup>, options: { frames?: number } = {}): StressBench {
...
export function registerAll(benches: Bench[]): void {
  for (const b of benches) registry.push(b);
}

function runCpuBench(bench: CpuBench): CpuResult {
...
async function runGpuBench(bench: GpuBench): Promise<GpuResult> {
...
async function runStressBench(bench: StressBench): Promise<StressResult> {
  ...
  let cpuEncodeAvg: number | null = null;
  if (tickCpuOnly) {
    const times: number[] = [];
...
function computeStats(name: string, samples: number[], iterations: number): CpuResult {
...
export async function runAll(outputEl: HTMLElement): Promise<BenchResult[]> {
...
function renderResult(r: BenchResult): HTMLElement {
  ...
  if (r.type === 'stress') {
    ...
    row.innerHTML = `
      ...
      <span style="color:#64748b">${r.iterations.toLocaleString()} x 30 runs</span>`;
  }
  return row;
}

function renderSummary(results: BenchResult[]): HTMLElement {
  const div = document.createElement('div');
  div.style.cssText = 'margin-top: 24px; padding: 12px 16px; background: #111122; border-radius: 4px; border: 1px solid #1e293b;';

  const gpu = results.filter((r): r is GpuResult => r.type === 'gpu');
  const cpu = results.filter((r): r is CpuResult => r.type === 'cpu');
