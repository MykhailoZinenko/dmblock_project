...
export class AtlasPacker {
  static pack(device, entries, maxSize) {
    if (!maxSize) maxSize = device.limits.maxTextureDimension2D || 2048;
    const sorted = entries.slice().sort((a, b) => b.bitmap.height - a.bitmap.height);
    ...
    for (const packer of atlases) {
      const w = packer.usedWidth;
      const h = packer.usedHeight;
      ...
      const baseTexture = new BaseTexture(gpuTexture, gpuSampler, w, h);
